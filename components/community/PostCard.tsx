...
export default function PostCard({ post, onDelete, locale = 'ko' }: {
  ...
  return (
    <div className="bg-white border border-stone-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition">
      {post.image_url && (
        <img src={post.image_url} alt="첨부 이미지" className="w-full h-44 object-cover" loading="lazy" />
      )}
      <div className="p-4">
        <span className="text-xs text-amber-600 font-medium">{post.category}</span>
        <p className="text-sm text-stone-700 mt-1 leading-relaxed line-clamp-4">{post.content}</p>
