...
function getDominantElement(energy: EnergyType) {
  const list = [
    { key: "목", value: energy.wood }, { key: "화", value: energy.fire },
    { key: "토", value: energy.earth }, { key: "금", value: energy.metal }, { key: "수", value: energy.water },
  ];
...
function getFortuneContent(result: ResultType) {
  ...
  return {
    overview: `이 사주는 일간 ${result.dayMaster}를 중심으로 볼 때 ${dayMasterElement}의 성향이 바탕을 이루고, 전체적으로는 ${result.dominantElement} 기운이 비교적 두드러지는 구조입니다. 즉, 기본 성향과 실제로 밖으로 드러나는 힘이 어느 정도 연결되어 작동할 가능성이 있으며, 강한 기운을 어떻게 안정적으로 쓰느냐가 전체 흐름의 핵심이 됩니다.`,
...
function PillarBlock({ item }: { item: PillarItem }) {
  return (
    <div className="flex flex-col items-center">
      ...
      <div className={`mb-2 flex h-16 w-16 items-center justify-center rounded-3xl border text-3xl font-bold shadow-sm ${getElementStyles(item.gan)}`}>{item.gan}</div>
      <div className="mb-3 text-[11px] font-medium text-zinc-500">{item.ganElement || "-"}</div>
      <div className={`mb-2 flex h-16 w-16 items-center justify-center rounded-3xl border text-3xl font-bold shadow-sm ${getElementStyles(item.ji)}`}>{item.ji}</div>
...
function SearchParamsLoader({ onLoad }: { onLoad: (data: BirthDataType) => void }) {
  ...
  useEffect(() => {
    ...
    if (year && month && day && lunar) {
      onLoad({ year, month, day, hour, isLunar: lunar, isLeapMonth: leap, unknownTime: hour ? "false" : "true" });
    }
  }, [searchParams, onLoad]);
  return null;
}
...
function SajuPageInner() {
  ...
  const [birthData, setBirthData] = useState<BirthDataType>({
    year: "", month: "", day: "", hour: "", isLunar: "", isLeapMonth: "", unknownTime: "",
  });
  ...
  // URL 파라미터에서 데이터 로드
  const handleUrlLoad = useCallback((data: BirthDataType) => {
    setBirthData(data);
  ...
  // 입력값 바뀌면 결과 초기화 (URL 로드 직후 제외)
  useEffect(() => {
    if (loadedFromUrl) { setLoadedFromUrl(false); return; }
    setResult(null);
    setErrorMessage("");
    setOpenedPanel(null);
  ...
  return (
    <main className="min-h-screen bg-[#f6f4ef] text-zinc-900">
      {/* ✅ Suspense 안에 SearchParamsLoader 배치 */}
      <Suspense fallback={null}>
        <SearchParamsLoader onLoad={handleUrlLoad} />
      </Suspense>
        ...
        <header className="sticky top-0 z-20 border-b border-black/5 bg-[#f6f4ef]/90 backdrop-blur">
          <div className="flex items-center justify-between px-4 py-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <button type="button" onClick={handleKakaoShare} disabled={!result}
              ...
              <section className="mt-4 rounded-[32px] bg-white p-5 shadow-[0_12px_40px_rgba(0,0,0,0.06)] ring-1 ring-zinc-100">
                ...
                <div className="mt-6 flex h-44 items-end justify-between gap-3 rounded-3xl bg-zinc-50 px-4 pb-4 pt-6">
                  {[
                    ...
                    { label: "토", value: result.energy.earth }, { label: "금", value: result.energy.metal }, { label: "수", value: result.energy.water },
                  ].map((item) => {
...
// ✅ 페이지 export — SajuPageInner를 Suspense로 감싸서 내보냄
