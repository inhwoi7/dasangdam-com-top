...
type CompatibilityType = {
  ...
  dayBranchChemistry: string;
  energyChemistry: string;
  caution: string;
  advice: string[];
  combinedEnergy: EnergyType;
};

const EMPTY_BIRTH_DATA: BirthDataType = {
  year: "",
  month: "",
  day: "",
  hour: "",
...
function calculatePersonResult(birthData: BirthDataType): ResultType {
  ...
  const basisText = isLunar
    ? `음력 입력값을 양력으로 변환한 뒤 계산했어요${
        unknownTime ? " · 시간 미입력으로 시주는 제외했어요" : ""
      }`
    : `입력한 양력 기준으로 계산했어요${
        unknownTime ? " · 시간 미입력으로 시주는 제외했어요" : ""
      }`;
...
function getElementRelation(a: string, b: string) {
  if (!a || !b || a === "-" || b === "-") {
    return { type: "neutral", score: 0, label: "판단 보류" };
  }

  if (a === b) {
    return { type: "same", score: 12, label: "같은 결" };
  }

  if (GENERATES[a] === b) {
    return { type: "a-generates-b", score: 18, label: `${a} → ${b} 상생` };
  }

  if (GENERATES[b] === a) {
    return { type: "b-generates-a", score: 18, label: `${b} → ${a} 상생` };
  }

  if (CONTROLS[a] === b) {
    return { type: "a-controls-b", score: -8, label: `${a} → ${b} 상극` };
  }

  if (CONTROLS[b] === a) {
    return { type: "b-controls-a", score: -8, label: `${b} → ${a} 상극` };
  }
...
function getBranchRelation(a: string, b: string) {
  ...
  if (a === b) {
    return { type: "same", score: 8, label: "같은 일지" };
  }

  if (BRANCH_HARMONY[a] === b) {
    return { type: "harmony", score: 15, label: "육합 성향" };
  }

  if (BRANCH_CLASH[a] === b) {
    return { type: "clash", score: -15, label: "충 성향" };
  }
...
function getCompatibilityContent(a: ResultType, b: ResultType): CompatibilityType {
  ...
  let balanceScore = 0;
  if (gap <= 1) balanceScore = 10;
  else if (gap <= 2) balanceScore = 8;
  else if (gap <= 3) balanceScore = 5;
  else if (gap <= 4) balanceScore = 2;

  let score =
    50 +
    dayMasterRelation.score +
    dominantRelation.score +
  ...
  const dayMasterChemistryMap: Record<string, string> = {
    ...
    neutral:
      "일간 기준으로는 강한 상생이나 상극보다 무난한 흐름에 가깝습니다. 결국 관계의 질은 대화 방식과 생활 리듬 조율에 더 크게 좌우됩니다.",
  };

  const dayBranchChemistryMap: Record<string, string> = {
    same: "일지가 같아 생활 감각이나 친밀감의 결이 비슷하게 맞아들어갈 가능성이 큽니다. 익숙함과 안정감이 장점이 되기 쉽습니다.",
    harmony:
      "일지 사이에 합 성향이 있어 일상적인 호흡, 정서적 친밀감, 함께 있을 때의 편안함이 잘 만들어질 수 있습니다.",
    clash:
      "일지 사이에 충 성향이 있어 가까워질수록 생활 습관, 감정 반응, 거리감 조절에서 부딪힘이 생길 수 있습니다. 대신 서로에게 강한 자극과 끌림으로 작용할 가능성도 있습니다.",
    neutral:
      "일지 기준으로는 특별히 강한 합이나 충보다는 무난한 흐름입니다. 관계의 핵심은 다른 요소에서 더 많이 결정됩니다.",
  };
  ...
  if (complementScore >= 10) {
    ...
    energyChemistry = `두 사람 모두 ${a.dominantElement} 기운이 강해 관계의 추진 방식이 비슷할 가능성이 큽니다. 함께 움직일 때는 빠르지만, 한쪽으로 과하게 쏠리지 않도록 균형을 잡는 것이 중요합니다.`;
  } else if (
    dominantRelation.type === "a-generates-b" ||
    dominantRelation.type === "b-generates-a"
  ) {
    energyChemistry =
      "대표 오행이 상생 관계라 함께 있을 때 흐름이 부드럽게 이어질 가능성이 큽니다. 서로의 강점을 자연스럽게 살려주면 관계 효율이 높아집니다.";
  } else if (
    dominantRelation.type === "a-controls-b" ||
    dominantRelation.type === "b-controls-a"
  ) {
    energyChemistry =
  ...
  if (dayBranchRelation.type === "clash") {
    ...
    dayMasterRelation.type === "a-controls-b" ||
    dayMasterRelation.type === "b-controls-a"
  ) {
    caution =
      "한쪽이 상대를 이끌거나 교정하려는 흐름이 강해지면 관계 피로가 빨리 쌓일 수 있습니다. 조언과 통제를 구분하는 태도가 필요합니다.";
  } else {
    caution =
      "기본적으로 무난한 흐름이더라도 오래 가는 관계는 결국 대화 습관과 현실적인 배려에서 차이가 납니다. 잘 맞는 부분만 믿기보다 생활 속 합을 만들어가는 태도가 중요합니다.";
  }

  const advice: string[] = [];

  if (
    dayMasterRelation.type === "a-generates-b" ||
    dayMasterRelation.type === "b-generates-a"
  ) {
    advice.push(
  ...
  return {
    ...
    relationLabel: gradeInfo.label,
    confidenceLabel,
    summary,
    dayMasterChemistry:
      dayMasterChemistryMap[dayMasterRelation.type] ||
      dayMasterChemistryMap.neutral,
    dayBranchChemistry:
      dayBranchChemistryMap[dayBranchRelation.type] ||
      dayBranchChemistryMap.neutral,
    energyChemistry,
    caution,
    advice,
    combinedEnergy,
...
function PillarBlock({ item }: { item: PillarItem }) {
  return (
    <div className="flex flex-col items-center">
      ...
      <div
        className={`mb-2 flex h-14 w-14 items-center justify-center rounded-3xl border text-2xl font-bold shadow-sm ${getElementStyles(
          item.gan
        )}`}
      >
        {item.gan}
    ...
    <section className="rounded-[32px] bg-white p-5 shadow-[0_12px_40px_rgba(0,0,0,0.06)] ring-1 ring-zinc-100">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <SelectField
          ...
          value={birthData.day}
          onChange={(value) =>
            setBirthData((prev) => ({ ...prev, day: value }))
          }
  ...
  return (
    <section className="rounded-[32px] bg-white p-5 shadow-[0_12px_40px_rgba(0,0,0,0.06)] ring-1 ring-zinc-100">
      <div className="flex items-start justify-between gap-3">
        ...
        <div
          className={`rounded-full border px-3 py-1.5 text-xs font-bold ${dominantMeta.chipClass}`}
        >
      ...
      <div className="mt-4 grid grid-cols-4 gap-2">
        {result.pillars.map((item) => (
          <PillarBlock key={`${title}-${item.label}`} item={item} />
        ))}
      </div>
      ...
      <div className="mt-4 grid grid-cols-3 gap-3">
        ...
        <div className="rounded-2xl bg-emerald-50 p-4 ring-1 ring-emerald-100">
          ...
          <div className="mt-1 text-base font-extrabold text-zinc-900">
            {dominantMeta.emoji} {result.dominantElement}
          </div>
        </div>
      </div>
...
function EnergyCompareSection({
  ...
  a: ResultType;
  b: ResultType;
  combined: CompatibilityType["combinedEnergy"];
}) {
  const rows = [
              ...
              <div className="grid grid-cols-[48px_1fr_48px] items-center gap-3">
                ...
                <div className="h-3 overflow-hidden rounded-full bg-white/80">
                  <div
                    ...
                    style={{ width: `${Math.min(100, row.sum * 10)}%` }}
                  />
                </div>
...
export default function Page() {
  ...
  const [compatibility, setCompatibility] =
    useState<CompatibilityType | null>(null);
  ...
  const canCalculate =
    canCalculateBirthData(personA) && canCalculateBirthData(personB);
  ...
  const handleCalculate = useCallback(() => {
    try {
      ...
      const calculatedCompatibility = getCompatibilityContent(
        calculatedA,
        calculatedB
      );

      setResultA(calculatedA);
      setResultB(calculatedB);
  ...
  const handleShare = async () => {
    try {
      ...
      const text = `궁합 결과\nA: ${resultA.summaryText}\nB: ${resultB.summaryText}\n궁합 점수: ${compatibility.score}점 (${compatibility.relationLabel})`;
      ...
      if (navigator.share) {
        ...
        window.alert("궁합 결과 요약이 클립보드에 복사되었습니다.");
      }
    } catch (error) {
      console.error(error);
    }
      ...
      <div className="mx-auto max-w-md pb-28">
        ...
        <div className="space-y-4 px-4 pt-4">
          ...
          <button
            ...
            className={`w-full rounded-[24px] px-5 py-4 text-sm font-extrabold text-white transition ${
              canCalculate
            ...
            <>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-1 text-sm font-bold text-rose-600">
                      {compatibility.relationLabel} · {compatibility.grade}
                    </div>
              ...
              <section className="grid grid-cols-3 gap-3">
                ...
                <SummaryCard
                  ...
                  value={compatibility.grade}
                  title="등급"
                  sub={compatibility.relationLabel}
                />
                <SummaryCard
                  icon={<Info className="h-5 w-5" />}
                  value={compatibility.confidenceLabel.split(" · ")[0]}
                  title="해석 신뢰도"
                  sub={compatibility.confidenceLabel}
                />
              </section>
              ...
              <EnergyCompareSection
                a={resultA}
                b={resultB}
                combined={compatibility.combinedEnergy}
              />

              <section className="rounded-[32px] bg-white p-5 shadow-[0_12px_40px_rgba(0,0,0,0.06)] ring-1 ring-zinc-100">
                <div className="text-lg font-extrabold">궁합 풀이</div>
                <div className="mt-1 text-sm text-zinc-500">
                  일간, 일지, 대표 오행과 전체 분포를 기준으로 자연스럽게 읽은
                  해석입니다
                </div>
                ...
                <div className="mt-4 rounded-3xl bg-rose-50 p-4 ring-1 ring-rose-100">
                  ...
                  <div className="mt-2 text-sm leading-7 text-zinc-700">
                    {compatibility.dayMasterChemistry}
                  </div>
                </div>
                ...
                <div className="mt-3 rounded-3xl bg-amber-50 p-4 ring-1 ring-amber-100">
                  ...
                  <div className="mt-2 text-sm leading-7 text-zinc-700">
                    {compatibility.dayBranchChemistry}
                  </div>
                </div>
          ...
          >
            ...
            <div className="mt-4 space-y-3">
              <div className="rounded-3xl bg-zinc-50 p-4">
                ...
                <div className="mt-2 text-sm leading-7 text-zinc-700">
                  사주 궁합은 두 사람의 기운 흐름과 조화를 참고용으로 해석한
                  내용입니다. 실제 관계는 서로의 배려와 대화 방식, 생활 습관에
                  따라 더 크게 달라질 수 있습니다.
                </div>
              </div>
              ...
              <div className="rounded-3xl bg-zinc-50 p-4">
                ...
                <div className="mt-2 text-sm leading-7 text-zinc-700">
                  태어난 시간을 모르면 시주를 제외하고 계산합니다. 이 경우 세부
                  성향보다는 큰 흐름 중심의 해석으로 보는 것이 자연스럽습니다.
                </div>
              </div>
              ...
              <div className="rounded-3xl bg-zinc-50 p-4">
                ...
                <div className="mt-2 text-sm leading-7 text-zinc-700">
                  음력 날짜를 입력한 경우에는 양력으로 변환한 뒤 사주를 계산합니다.
                  윤달 여부까지 정확하게 선택해야 결과가 달라지지 않습니다.
                </div>
              </div>
              ...
              <button
                type="button"
                onClick={() => setInfoOpen(false)}
                className="mt-2 w-full rounded-[24px] bg-zinc-900 px-5 py-4 text-sm font-extrabold text-white"
              >
                닫기
              </button>
            </div>
