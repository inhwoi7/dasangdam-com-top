...
// 시간 없어도 계산 가능 - year/month/day/isLunar만 필수
...
function calculatePersonResult(birthData: BirthDataType): ResultType {
  ...
  return {
    ...
    yearPillarText, monthPillarText, dayPillarText, hourPillarText, unknownTime,
  };
}

function mergeEnergy(a: EnergyType, b: EnergyType): EnergyType {
  return {
    wood: a.wood + b.wood, fire: a.fire + b.fire, earth: a.earth + b.earth,
    metal: a.metal + b.metal, water: a.water + b.water,
  };
}
...
function getCompatibilityContent(a: ResultType, b: ResultType): CompatibilityType {
  ...
  if (complementScore >= 10) {
    energyChemistry = "오행 분포를 보면 한쪽의 강한 기운이 다른 쪽의 약한 부분을 채워주는 보완성이 비교적 잘 보입니다. 함께 있을 때 서로에게 없는 리듬을 공급하는 구조로 작동할 수 있습니다.";
    ...
    energyChemistry = "대표 오행이 상생 관계라 함께 있을 때 흐름이 부드럽게 이어질 가능성이 큽니다. 서로의 강점을 자연스럽게 살려주면 관계 효율이 높아집니다.";
  } else if (dominantRelation.type === "a-controls-b" || dominantRelation.type === "b-controls-a") {
    energyChemistry = "대표 오행 사이에 상극 흐름이 있어 결정 방식이나 우선순위에서 긴장감이 생길 수 있습니다. 한 사람이 주도권을 독점하면 피로가 커질 수 있으니, 번갈아 리드하는 구조가 좋습니다.";
  } else {
    energyChemistry = "대표 오행 기준으로는 강한 보완이나 강한 충돌보다 중간 흐름에 가깝습니다. 관계의 안정성은 감정 표현과 실질적인 생활 조율에 달려 있습니다.";
  }

  let caution = "";
  if (dayBranchRelation.type === "clash") {
    caution = "가까워질수록 사소한 생활 방식, 연락 빈도, 감정 표현 속도에서 부딪힐 수 있습니다. 문제의 원인을 성격 탓으로만 보지 말고 리듬 차이로 이해하는 것이 중요합니다.";
  } else if (dayMasterRelation.type === "a-controls-b" || dayMasterRelation.type === "b-controls-a") {
    caution = "한쪽이 상대를 이끌거나 교정하려는 흐름이 강해지면 관계 피로가 빨리 쌓일 수 있습니다. 조언과 통제를 구분하는 태도가 필요합니다.";
  } else {
    caution = "기본적으로 무난한 흐름이더라도 오래 가는 관계는 결국 대화 습관과 현실적인 배려에서 차이가 납니다. 잘 맞는 부분만 믿기보다 생활 속 합을 만들어가는 태도가 중요합니다.";
  }

  const advice: string[] = [];

  if (dayMasterRelation.type === "a-generates-b" || dayMasterRelation.type === "b-generates-a") {
    advice.push("한쪽이 주는 에너지와 다른 한쪽이 받는 에너지의 흐름이 좋으니, 고마움 표현을 자주 해주는 것이 관계 안정에 도움이 됩니다.");
  }
  if (dayMasterRelation.type === "a-controls-b" || dayMasterRelation.type === "b-controls-a") {
    advice.push("의견 차이가 생길 때는 누가 맞는지보다 어떤 방식이 더 편한지부터 확인하면 갈등이 줄어듭니다.");
  }
  if (dayBranchRelation.type === "harmony") {
    advice.push("함께 보내는 시간을 규칙적으로 확보하면 정서적 친밀감이 더 빠르게 쌓일 수 있습니다.");
  }
  if (dayBranchRelation.type === "clash") {
    advice.push("연락 주기, 혼자 있는 시간, 소비 습관처럼 일상 규칙을 미리 맞춰두는 것이 좋습니다.");
  }
  if (aWeak.includes(b.dominantElement) || bWeak.includes(a.dominantElement)) {
    advice.push("서로의 강점이 상대의 빈 부분을 채워주는 면이 있으니, 바꾸려 하기보다 맡기고 기대는 구조를 만드는 것이 좋습니다.");
  }
  if (a.unknownTime || b.unknownTime) {
    advice.push("태어난 시간을 모르는 쪽은 시주를 제외해 해석했으므로, 세부 생활 성향보다는 큰 결의 궁합 중심으로 보는 것이 자연스럽습니다.");
  }
  if (advice.length === 0) {
    advice.push("궁합은 기본적으로 나쁘지 않지만, 실제 만족도는 관계 운영 방식에 더 크게 좌우됩니다.");
  }

  return {
    score, grade: gradeInfo.grade, relationLabel: gradeInfo.label,
...
function BirthFormCard({
  title, birthData, setBirthData, years, months, hours,
}: {
  ...
  return (
    <section className="rounded-[32px] bg-white p-5 shadow-[0_12px_40px_rgba(0,0,0,0.06)] ring-1 ring-zinc-100">
      ...
      <div className="grid grid-cols-3 gap-3">
        <SelectField label="태어난 년" value={birthData.year}
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <SelectField label="태어난 시간 (선택)" value={birthData.hour}
          onChange={(value) => setBirthData((prev) => ({ ...prev, hour: value }))}
      ...
      <div className="mt-4">
        {birthData.isLunar === "lunar" ? (
          <SelectField label="윤달 여부" value={birthData.isLeapMonth}
            onChange={(value) => setBirthData((prev) => ({ ...prev, isLeapMonth: value as "" | "false" | "true" }))}>
            <option value="">선택</option>
...
export default function Page() {
  ...
  const handleShare = async () => {
    try {
      ...
      if (navigator.share) {
        await navigator.share({ title: "사주 궁합 결과", text });
      } else {
        ...
        <header className="sticky top-0 z-20 border-b border-black/5 bg-[#f6f4ef]/90 backdrop-blur">
          <div className="flex items-center justify-between px-4 py-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <button type="button" onClick={handleShare} className="flex h-10 w-10 items-center justify-center rounded-full bg-white shadow-sm ring-1 ring-zinc-200" aria-label="공유">
