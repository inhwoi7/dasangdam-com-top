...
> = {
  외향성: {
    glow: 'from-orange-400 via-rose-400 to-pink-500',
    ring: 'ring-orange-200',
    card: 'from-orange-50 via-rose-50 to-pink-50',
    chip: 'bg-orange-100 text-orange-700',
    bar: 'from-orange-400 via-rose-500 to-pink-500',
    text: 'text-orange-700',
  },
  친화성: {
    glow: 'from-sky-400 via-cyan-400 to-teal-500',
    ring: 'ring-cyan-200',
    card: 'from-sky-50 via-cyan-50 to-teal-50',
    chip: 'bg-cyan-100 text-cyan-700',
    bar: 'from-sky-400 via-cyan-500 to-teal-500',
    text: 'text-cyan-700',
  },
  성실성: {
    glow: 'from-violet-400 via-fuchsia-400 to-purple-500',
    ring: 'ring-violet-200',
    card: 'from-violet-50 via-fuchsia-50 to-purple-50',
    chip: 'bg-violet-100 text-violet-700',
    bar: 'from-violet-400 via-fuchsia-500 to-purple-500',
    text: 'text-violet-700',
  },
  신경증: {
    glow: 'from-rose-400 via-red-400 to-amber-400',
    ring: 'ring-rose-200',
    card: 'from-rose-50 via-red-50 to-amber-50',
    chip: 'bg-rose-100 text-rose-700',
    bar: 'from-rose-400 via-red-500 to-amber-400',
    text: 'text-rose-700',
  },
  개방성: {
    glow: 'from-emerald-400 via-lime-400 to-green-500',
    ring: 'ring-emerald-200',
    card: 'from-emerald-50 via-lime-50 to-green-50',
    chip: 'bg-emerald-100 text-emerald-700',
    bar: 'from-emerald-400 via-lime-500 to-green-500',
...
export default function BigFiveTestPage() {
  ...
  const scores = useMemo(() => {
    const grouped: Record<Factor, number[]> = {
      외향성: [],
      친화성: [],
      성실성: [],
      신경증: [],
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(125,211,252,0.25),_transparent_22%),radial-gradient(circle_at_top_right,_rgba(244,114,182,0.20),_transparent_24%),radial-gradient(circle_at_bottom_left,_rgba(192,132,252,0.18),_transparent_24%),linear-gradient(180deg,_#fff8fb_0%,_#f5f7ff_38%,_#eefaf7_100%)]">
      <div className="mx-auto max-w-6xl space-y-6">
        <section className="relative overflow-hidden rounded-[32px] border border-white/60 bg-white/70 p-6 shadow-[0_20px_60px_rgba(15,23,42,0.10)] backdrop-blur-xl md:p-8">
          ...
          <div className="absolute bottom-0 right-20 h-36 w-36 rounded-full bg-emerald-300/20 blur-3xl" />
          ...
          <div className="relative flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
            <div className="max-w-3xl">
              <div className="inline-flex items-center rounded-full border border-white/70 bg-white/80 px-3 py-1 text-xs font-semibold tracking-wide text-slate-500 shadow-sm">
                Big Five Personality Dashboard
              </div>
              <h1 className="mt-4 text-3xl font-black tracking-tight text-slate-900 md:text-5xl">
                Big Five 성격검사 앱
              </h1>
              <p className="mt-3 text-sm leading-7 text-slate-600 md:text-base">
                앱스토어 스타일의 둥근 카드, 파스텔 그라데이션, 부드러운 그림자, 직관적인 선택 버튼을 적용한 성격검사 UI입니다. 역문항(R)은 자동으로 계산됩니다.
              </p>
            </div>
        ...
        {submitted && topFactor && (
          <section className="relative overflow-hidden rounded-[32px] border border-white/60 bg-white/70 p-6 shadow-[0_24px_80px_rgba(15,23,42,0.12)] backdrop-blur-xl md:p-8">
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                <div className="inline-flex rounded-full bg-slate-900 px-3 py-1 text-xs font-semibold text-white shadow-lg">
                  결과 분석 완료
                </div>
                <h2 className="mt-4 text-3xl font-black text-slate-900">당신의 성향 프로파일</h2>
                <p className="mt-2 text-sm leading-7 text-slate-600">
                  가장 높게 나온 성향은 <span className={cls('font-bold', factorStyles[topFactor.factor].text)}>{topFactor.factor}</span>이며,
              ...
              <div className="flex gap-3">
                <button
                  onClick={handleReset}
                  className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg"
                >
                  다시 하기
                </button>
                <button
                  onClick={() => window.print()}
                  className="rounded-2xl bg-gradient-to-r from-slate-900 to-slate-700 px-4 py-3 text-sm font-semibold text-white shadow-[0_10px_25px_rgba(15,23,42,0.25)] transition hover:-translate-y-0.5"
                >
                  결과 저장
                </button>
              </div>
              ...
              {scores.map((item, index) => {
                ...
                return (
                  <div
                    key={item.factor}
                    className={cls(
                      'relative overflow-hidden rounded-[28px] border bg-gradient-to-br p-5 shadow-[0_18px_45px_rgba(15,23,42,0.10)] transition hover:-translate-y-1 hover:shadow-[0_24px_60px_rgba(15,23,42,0.16)]',
                      style.card,
                      style.ring,
                      isTop && 'scale-[1.02]'
                    )}
                  >
                    ...
                    {isTop && (
                      <div className="absolute right-4 top-4 rounded-full bg-white/80 px-3 py-1 text-[11px] font-bold text-slate-900 shadow-sm">
                        TOP 1
                      </div>
                    )}

                    <div className={cls('inline-flex rounded-full px-3 py-1 text-xs font-bold shadow-sm', style.chip)}>
                      {item.factor}
                    </div>
                    ...
                    <div className="mt-5 h-4 overflow-hidden rounded-full bg-white/80 p-1 shadow-inner">
                      <div
                        className={cls('h-full rounded-full bg-gradient-to-r shadow-[0_8px_18px_rgba(59,130,246,0.25)]', style.bar)}
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-[1.15fr_0.85fr]">
              <div className="rounded-[28px] border border-amber-200 bg-gradient-to-br from-amber-50 to-orange-50 p-5 shadow-sm">
                ...
                <p className="mt-2 text-sm leading-7 text-amber-900">
                  이 결과는 참고용 자기이해 자료입니다. 의료적 진단이나 전문 심리평가를 대체하지 않으며,
                  응답 당시의 상태나 환경에 따라 달라질 수 있습니다.
                </p>
              </div>
              ...
              <div className="rounded-[28px] border border-slate-200 bg-white/80 p-5 shadow-sm">
                ...
                <ul className="mt-3 space-y-2 text-sm leading-6 text-slate-600">
                  ...
                  <li>• 검사 완료도: <span className="font-bold text-slate-900">100%</span></li>
        ...
        <section className="space-y-5">
          {questions.map((q) => {
            ...
            return (
              <article
                key={q.id}
                className="relative overflow-hidden rounded-[30px] border border-white/70 bg-white/75 p-5 shadow-[0_14px_40px_rgba(15,23,42,0.08)] backdrop-blur-xl transition hover:-translate-y-0.5 hover:shadow-[0_20px_50px_rgba(15,23,42,0.12)] md:p-6"
              >
                  ...
                  <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className={cls('rounded-full px-3 py-1 text-xs font-bold shadow-sm', style.chip)}>
                          {q.factor}
                        </span>
                        {q.reverse && (
                          <span className="rounded-full bg-rose-100 px-3 py-1 text-xs font-bold text-rose-700 shadow-sm">
                            역채점 문항
                          </span>
                        )}
                      </div>
                    ...
                    {choices.map((choice) => {
                      ...
                      return (
                        <label
                          key={choice.value}
                          className={cls(
                            'group cursor-pointer rounded-[24px] border px-4 py-4 text-center transition-all duration-200',
                            active
                              ? `border-transparent bg-gradient-to-br ${style.glow} text-white shadow-[0_14px_30px_rgba(59,130,246,0.22)] scale-[1.02]`
                              : 'border-slate-200 bg-white/90 text-slate-700 shadow-sm hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-md'
                          )}
                        >
                          <input
                            type="radio"
                            ...
                            value={choice.value}
                            checked={active}
                            onChange={() => handleSelect(q.id, choice.value)}
                          />
                          <div className="text-lg font-black">{choice.value}</div>
                          <div className={cls('mt-1 text-xs font-medium', active ? 'text-white/90' : 'text-slate-500')}>
                            {choice.label}
                          </div>
                        </label>
                      );
          ...
          <div className="overflow-hidden rounded-[28px] border border-white/60 bg-white/80 p-4 shadow-[0_20px_50px_rgba(15,23,42,0.14)] backdrop-blur-xl md:p-5">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              ...
              <div className="flex items-center gap-3">
                <button
                  onClick={handleReset}
                  className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
                >
                  초기화
                </button>
                <button
                  onClick={handleSubmit}
                  className="rounded-2xl bg-gradient-to-r from-sky-500 via-violet-500 to-fuchsia-500 px-6 py-3 text-sm font-bold text-white shadow-[0_14px_30px_rgba(99,102,241,0.32)] transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-40"
                >
                  결과 보기
                </button>
              </div>
